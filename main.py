...
def determine_half_partition(chars, line):
    diff = line[1] - 32
    if chars.get(line[0]):
...
def sort():
    ...
    with FunctionExecutor(workers=max_parallelism, runtime='bogdan/radix-sorting-container-100-mb-files') as fexec:
        ...
        for i in range(number_of_lambda_sessions):
            determine_categories_futures = fexec.map(determine_categories,
                                                     current_keys_list[
                                                                   i * (len(current_keys_list) // number_of_lambda_sessions):
                                                                   (i+1) * (len(current_keys_list) // number_of_lambda_sessions)])
        ...
        # Process remainings
        if len(current_keys_list) % number_of_lambda_sessions != 0:
            determine_categories_futures = fexec.map(determine_categories,
                                                     current_keys_list[
                                                     (i + 1) * (len(current_keys_list) // number_of_lambda_sessions):])
            determine_categories_result = fexec.get_result(determine_categories_futures)
        ...
        # categories_keys_dict = [{'category_key_name': '2gb-input'}]
        for i in range(number_of_lambda_sessions):
            sort_categories_futures = fexec.map(sort_category, categories_keys_dict[
                                                            i * (len(categories_keys_dict) // number_of_lambda_sessions):
                                                            (i+1) * (len(categories_keys_dict) // number_of_lambda_sessions)])
